...
def match_detections_to_tracks(track_boxes, det_boxes, threshold=0.8):
...
class Track:
    ...
    def update(self, new_box, new_class_id, new_score):
        ...
        self.hits += 1
        self.lost = 0
        new_velocity = self.box - old_box
        self.velocity = 0.6 * new_velocity + 0.4 * self.velocity
...
class Tracker:
    ...
    def update(self, boxes, scores, class_ids):
        ...
        for t in self.lost_tracks:
            t.predict()
        high_boxes, high_cls, high_scores = [], [], []
        low_boxes,  low_cls,  low_scores  = [], [], []

        for box, cls, score in zip(boxes, class_ids, scores):
            if score >= HIGH_SCORE:
                high_boxes.append(box)
                high_cls.append(cls)
        ...
        track_boxes = [t.box.tolist() for t in all_tracks]

        matched, unmatched_track_ids, unmatched_det_ids = match_detections_to_tracks(
            track_boxes, high_boxes, threshold=0.8
        )

        for track_idx, det_idx in matched:
            all_tracks[track_idx].update(
                high_boxes[det_idx], high_cls[det_idx], high_scores[det_idx]
            )
        ...
        for i in unmatched_track_ids:
            track = all_tracks[i]
            if track.lost == 0:  
                leftover_active.append(track)
            else:
                leftover_lost.append(track)

        if leftover_active and low_boxes:
            leftover_boxes = [t.box.tolist() for t in leftover_active]
            matched2, still_unmatched, _ = match_detections_to_tracks(
                leftover_boxes, low_boxes, threshold=0.8
            )

            for track_idx, det_idx in matched2:
                leftover_active[track_idx].update(
                    low_boxes[det_idx], low_cls[det_idx], low_scores[det_idx]
                )

            for i in still_unmatched:
                leftover_active[i].mark_lost()
        else:
            for t in leftover_active:
                t.mark_lost()

        # Mark leftover lost tracks as still lost
        for t in leftover_lost:
            t.mark_lost()

        for det_idx in unmatched_det_ids:
            if high_scores[det_idx] >= HIGH_SCORE:
                new_track = Track(high_boxes[det_idx], high_cls[det_idx], high_scores[det_idx])
        ...
        for t in all_tracks:
            if t.lost == 0:
                new_active.append(t)
            elif t.lost <= MAX_LOST:
                new_lost.append(t)
            
        for t in self.active_tracks:
            if t not in new_active and t not in new_lost and t.lost == 0:
                new_active.append(t)
...
count_log.write("frame,unique_vehicles,unique_pedestrians,unique_total\n")
...
with torch.no_grad():
    while True:
        ...
        for cls in set(class_ids):
            ...
            kept = nms(cb, cs)
            for k in kept:
                x1 = int(cb[k][0] * W / IMG_SIZE)
                y1 = int(cb[k][1] * H / IMG_SIZE)
                x2 = int(cb[k][2] * W / IMG_SIZE)
                y2 = int(cb[k][3] * H / IMG_SIZE)
                x1, y1 = max(0, x1), max(0, y1)
                x2, y2 = min(W, x2), min(H, y2)
                if x2 > x1 and y2 > y1:
                    final_boxes.append([x1, y1, x2, y2])
                    final_cls.append(cls)
                    final_scores.append(cs[k])

        active = tracker.update(final_boxes, final_scores, final_cls)

        for t in active:
            if not t.counted and t.hits >= CONFIRM_AGE:
                t.counted = True
                if t.class_id in VEHICLE_IDS:
                    vehicle_count += 1
                else:
                    pedestrian_count += 1
        ...
        count_log.write(
            f"{frame_count},{vehicle_count},{pedestrian_count},"
            f"{vehicle_count+pedestrian_count}\n"
        )
        ...
        for t in active:
            ...
            # Orange for vehicles, green for pedestrians
            color = (0, 200, 255) if t.class_id in VEHICLE_IDS else (0, 255, 100)

            thickness = 2 if t.counted else 1
            cv2.rectangle(frame, (x1, y1), (x2, y2), color, thickness)

            if t.counted:
                label = f"#{t.id} {CLASS_NAMES[t.class_id]}"
...
print(f"Unique vehicles seen:    {vehicle_count}")
print(f"Unique pedestrians seen: {pedestrian_count}")
print(f"Total unique objects:    {vehicle_count + pedestrian_count}")
